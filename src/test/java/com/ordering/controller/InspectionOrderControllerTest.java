...
public class InspectionOrderControllerTest {

  InspectionOrder sampleInspectionOrder;
  List<InspectionOrder> mockInspectionOrderList;
  @Autowired
  MockMvc mockMvc;
  ...
  @BeforeEach
  void setUp() {
    sampleInspectionOrder = new InspectionOrder(
        "id",
        "検査",
        "2024-12-22 11:05:15",
        "検査の詳細",
  ...
  void test_GET_index() throws Exception {
    mockInspectionOrderList = List.of(
        new InspectionOrder(
            "id1",
            "検査1",
            "2024-01-01 10:00:00",
        ...
        new InspectionOrder(
            ...
            null,
            null));
    Mockito.when(orderService.findAll()).thenReturn(mockInspectionOrderList);
    ...
    mockMvc.perform(
        ...
        .andExpect(model().attribute("inspections", mockInspectionOrderList))
  ...
  void test_GET_inspectionDetails() throws Exception {
    Mockito.when(orderService.findById("id")).thenReturn(sampleInspectionOrder);
    mockMvc.perform(
            get("/inspectionDetails/{id}", sampleInspectionOrder.getId())
                .with(csrf())
                .with(user("test").password("testTest"))
        )
        .andExpect(status().is2xxSuccessful())
        .andExpect(view().name("inspectionDetails"))
        .andExpect(model().attribute("inspection", sampleInspectionOrder));
  }
  ...
  void test_GET_edit() throws Exception {
    Mockito.when(orderService.findById("id")).thenReturn(sampleInspectionOrder);
    mockMvc.perform(
            get("/edit/{id}", sampleInspectionOrder.getId())
                .with(csrf())
                .with(user("test").password(("pass")))
        )
        .andExpect(status().is2xxSuccessful())
  ...
  void test_POST_newSubmitOk() throws Exception {
    ...
    doReturn(sampleInspectionOrder).when(orderService).findById(any(String.class));
    mockMvc.perform(
  ...
  void test_POST_editSubmitOK() throws Exception {
    doNothing().when(orderService)
        .edit(any(InspectionOrder.class), any(Authentication.class));
    doReturn(sampleInspectionOrder).when(orderService).findById(any(String.class));
    mockMvc.perform(
    ...
    verify(orderService, times(1)).edit(any(InspectionOrder.class),
        any(Authentication.class));
  }
  ...
  void test_GET_delete() throws Exception {
    Mockito.when(orderService.findById("id")).thenReturn(sampleInspectionOrder);
    mockMvc.perform(
            get("/delete/{id}", sampleInspectionOrder.getId())
                .with(csrf())
                .with(user("test").password("pass"))
        )
        .andExpect(status().is2xxSuccessful())
